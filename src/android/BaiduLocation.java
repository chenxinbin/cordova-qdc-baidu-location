    ...
    public BDLocationListener myListener = new BDLocationListener() {
        ...
        public void onReceiveLocation(BDLocation location) {
            try {
                ...
                if (location.getLocType() == BDLocation.TypeGpsLocation){// GPS定位结果
                ...
                } else if (location.getLocType() == BDLocation.TypeCacheLocation) {// 定位缓存的结果
                    sb.append("\ndescribe : ");
                    sb.append("离线定位成功，离线定位结果也是有效的");
                    json.put("status", "SUCCESS");
                    json.put("describe", "定位缓存的结果");
                } else if (location.getLocType() == BDLocation.TypeServerError) {
                    sb.append("\ndescribe : ");
                    sb.append("服务端网络定位失败，可以反馈IMEI号和大体定位时间到loc-bugs@baidu.com，会有人追查原因");

                    json.put("status", "FAILED");
                    json.put("describe", "服务端定位失败，请您检查是否禁用获取位置信息权限，尝试重新请求定位。");
                } else if (location.getLocType() == BDLocation.TypeNetWorkException) {
                    sb.append("\ndescribe : ");
                    sb.append("网络不同导致定位失败，请检查网络是否通畅");

                    json.put("describe", "网络异常，没有成功向服务器发起请求，请确认当前测试手机网络是否通畅，尝试重新请求定位。");
                    ...
                    json.put("describe", "无法获取有效定位依据，定位失败，请检查运营商网络或者WiFi网络是否正常开启，尝试重新请求定位。");
                }
    //            sb.append("\nlocationdescribe : ");
